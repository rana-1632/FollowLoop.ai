...
export class AiFallbackService {
  /**
   * Smart, context-aware fallback parser for interaction notes when OpenAI API calls fail (e.g. quota limits or missing key).
   */
  static fallbackParseInteraction(text: string, referenceDate: string): ParseInteractionResponseDto & { sequence1?: any; sequence2?: any } {
    const lower = text.toLowerCase();
    let channel: Channel = 'EMAIL';
    if (lower.includes('linkedin')) channel = 'LINKEDIN';
    else if (lower.includes('whatsapp') || lower.includes('chat') || lower.includes('message')) channel = 'WHATSAPP';
    ...
    // 3. Dynamic Contact Name Extraction (Zero static dummy fallbacks)
    let contactName: string | null = null;

    // Check verb/preposition pattern: "Met with Alex", "Spoke with Sarah", "Called Diego"
    const verbMatch = text.match(
      /(?:met with|spoke with|spoke to|called|emailed|contacted|meeting with|discussion with|note from|message from|talked to|attn:?|attention:?)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i,
    );
    ...
    // 3. Natural, Context-Aware Initial Draft (Step 1: Day 0)
    ...
    // 4. Multi-Step Follow-Up Sequence (Step 2: Day 3, Step 3: Day 7)
    const sequence1 = {
      step: 1,
  ...
  /**
   * Deterministic sequence variations when OpenAI API calls fail.
   */
  static fallbackSequenceVariations(contactName: string, company: string): GenerateSequenceResponseDto & { isFallback?: boolean; generationEngine?: string } {
    return {
      sequence1: {
        step: 1,
        name: 'Gentle Check-in',
  ...
  /**
   * Deterministic email draft generator when OpenAI API calls fail.
   */
  static generateFallbackDraft(recipientName: string, company: string, purpose?: string, contactId?: string): GenerateEmailResponseDto & { generationEngine?: string } {
    return {
      subject: `Quick check-in regarding ${purpose || 'our previous conversation'}`,
      body: `Hi ${recipientName},\n\nI hope you're having a great week.\n\nI wanted to briefly follow up regarding ${
        purpose || 'potential collaboration'
  ...
  static fallbackPostReplySequence(replyText: string, contactName: string = 'Prospect') {
    return {
  ...
  private static calculateDefaultFollowUpDate(baseDateStr: string, addDays: number): string {
    try {
